...
export const getOperationColumns = () => {
  return {
    ...
    showOverflow: 'title',
    formatter: ({ row }) => {
      return i18n.tm('cloudenvScheduledtaskRuleAction')[row.operation] || '-'
    },
  }
}
...
export const getResourceTypeColumns = () => {
  return {
    ...
    showOverflow: 'title',
    formatter: ({ row }) => {
      return i18n.tm('cloudenvScheduledtaskResourceType')[row.resource_type] || '-'
    },
  }
}
...
export const getLabelTypeColumns = () => {
  return {
    ...
    showOverflow: 'title',
    formatter: ({ row }) => {
      return i18n.tm('cloudenvScheduledtaskLabelType')[row.label_type] || '-'
    },
  }
}
...
export const getCycleTypeCloumns = () => {
  return {
    ...
    formatter: ({ row }) => {
      ...
      return i18n.tm('cloudenvScheduledtaskGroupCycleType')[row.cycle_timer.cycle_type]
    },
  }
}
    ...
    slots: {
      default: ({ row }, h) => {
        ...
        if (row.scheduled_type === 'cycle') {
          const cycleType = i18n.tm('cloudenvScheduledtaskGroupCycleType')[row.cycle_timer.cycle_type]
          ...
          if (row.cycle_timer.cycle_type === 'day') {
            ...
            const weekDays = row.cycle_timer.week_days.map((v) => {
              return i18n.tm('flexGroupSubCycleTypeWeek')[v]
            })
