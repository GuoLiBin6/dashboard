  ...
  <a-form
    ...
    <a-form-item label="webhook">
      <template v-slot:extra>
        <div>{{$t('system.text_265')}}</div>
        <div class="mb-0">{{$t('system.dingdingrobot.help')}}</div>
      </template>
  ...
  methods: {
    ...
    async handleSubmit () {
      ...
      try {
        ...
        this.$message.success(this.$t('system.text_124', [this.$t('system.text_303')]))
      } catch (error) {
        this.$message.error(this.$t('common_622', [this.$t('system.text_303')]))
        throw error
      } finally {
