<template>
  <base-dialog @cancel="cancelDialog">
    <template #header>{{ $t('common.application_scope') }}</template>
    <template #body>
      <dialog-selected-tips :count="params.data.length" :name="params.name || $t('common_92')" :action="$t('common.application_scope')" />
      ...
      <a-form-model
        ...
        <a-form-model-item :label="$t('common.application_scope')" prop="type">
...
export default {
  name: 'SetApplicationScopeDialog',
  ...
  data () {
      ...
      if (publicScope === 'system' && this.$store.getters.isAdminMode) {
        if (this.$store.getters.l3PermissionEnable) {
          ...
          if (R.isNil(sharedDomains) || R.isEmpty(sharedDomains)) {
            ...
            sharedDomainsInitialValue = sharedDomains.map(item => typeof item === 'object' ? item.id : item)
          }
        } else {
          ...
          if (R.isNil(sharedProjects) || R.isEmpty(sharedProjects)) {
            ...
            sharedProjectsInitialValue = sharedProjects.map(item => typeof item === 'object' ? item.id : item)
          }
        }
      }
      // 为域共享时
    ...
    return {
      ...
      rules: {
        type: [
          { required: true, message: this.$t('common.tips.select', [this.$t('common.application_scope')]) },
        ],
        shared_domains: [
