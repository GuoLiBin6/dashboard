<template>
  <div>
    ...
    <a-row :gutter="8" v-else>
      <a-col :span="12">
        <a-form-model-item :class="{ 'mb-0': mb0 }" v-if="isAdminMode && l3PermissionEnable" :wrapperCol="{ span: 24 }" :prop="domainProp">
          <base-select
            ref="domain"
            v-model="form.domain"
            resource="domains"
      ...
      <a-col :span="(isAdminMode && l3PermissionEnable) ? 12 : 24">
        <a-form-model-item :class="{ 'mb-0': mb0 }" :wrapperCol="{ span: 24 }" :prop="projectProp">
          <base-select
            ref="project"
  ...
  methods: {
    ...
    _setInitDomain (domain) {
      if (!R.isNil(domain) && !R.isEmpty(domain)) {
        if (this.labelInValue) {
          this.form[this.domainProp] = { key: domain.key, label: domain.label }
        } else {
          this.form[this.domainProp] = domain.key
        }
      }
    },
    /*
    ...
    _setInitProject (project) {
      if (!R.isNil(project) && !R.isEmpty(project)) {
        if (this.labelInValue) {
          this.form[this.projectProp] = { key: project.key, label: project.label }
        } else {
          this.form[this.projectProp] = project.key
        }
      }
    },
